...
//		String ATTACHES_DIR = request.getContextPath() + "/img";
		...
		System.out.println("진짜 경로 : " + ATTACHES_DIR);
		...
		if (contentType != null && contentType.toLowerCase().startsWith("multipart/")) {
			...
			for (Part part : parts) {
				System.out.printf("파라미터 명 : %s, contentType :  %s,  size : %d bytes \n", part.getName(),
				 ...
				 System.out.println("==========================================================");
				 Collection<String> strs = part.getHeaderNames();
				 for(String str : strs) {
					 System.out.println(str);
				 }
				 System.out.println("==========================================================");
				System.out.println("[ names of the part's Header ]  : " + part.getHeader("Content-Disposition"));
				
				if (part.getHeader("Content-Disposition").contains("filename=")) {
					String fileName = extractFileName(part.getHeader("Content-Disposition")); // extractFileName()
					System.out.println("fileName : " + fileName);
					if (part.getSize() > 0) {
						System.out.printf("업로드 파일 명 : %s  \n", fileName);
						part.write(ATTACHES_DIR + File.separator + fileName);
						part.delete(); // temp의 저장된 것들을 삭제
					}
				} else {
