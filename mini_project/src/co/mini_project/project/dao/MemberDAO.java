...
public class MemberDAO extends DAO {
	...
	public int memberInsert(MemberVO vo) { // 회원가입
		int n = 0;
		String sql = "INSERT INTO MEMBER(MNUMBER, MID, MPASSWORD, MNAME, MBIRTH, MTEL, MPOSTCODE, MADDRESS, MDETAILADR, MEMAIL) "
		...
		try {
			...
			psmt.setString(6, vo.getmPostcode());
			psmt.setString(7, vo.getmAddress());
			psmt.setString(8, vo.getmDetailAdr());
			psmt.setString(9, vo.getmEmail());
	...
	public MemberVO myEditList(MemberVO vo) { // 내 정보
		...
		try {
			...
			if (rs.next()) {
				...
				vo.setmTel(rs.getString("mTel"));
				vo.setmAddress(rs.getString("mAddress"));
				vo.setmDetailAdr(rs.getString("mDetailAdr"));
				vo.setmPostcode(rs.getString("mPostcode"));
				vo.setmEmail(rs.getString("mEmail"));
			}
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public int myEdit(MemberVO vo) { // 내 정보 수정
		int n = 0;
		String sql = "UPDATE MEMBER SET MNAME = ?, MPASSWORD = ?, MBIRTH = ?, MTEL = ?, MPOSTCODE = ?, MADDRESS = ?, MDETAILADR = ?, MEMAIL = ? "
				   + "WHERE MID = ?";
		try {
			psmt = conn.prepareStatement(sql);
			psmt.setString(1, vo.getmName());
			...
			psmt.setString(5, vo.getmPostcode());
			psmt.setString(6, vo.getmAddress());
			psmt.setString(7, vo.getmDetailAdr());
			psmt.setString(8, vo.getmEmail());
